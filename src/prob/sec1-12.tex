...
\begin{defn}
    ...
    \begin{equation*}
        \MyPr\biggl(\left\{\omega \colon \lim\limits _{n \to \infty} \xi_{n}(\omega)=\xi(\omega)\right\}\biggr)=1.
    \end{equation*}
\end{defn}
...
\begin{defn}
    Последовательность случайных величин $\{\xi_n\}_{n=1}^{+\infty}$ \\
    \textit{сходится в среднем} к случайной величине $\xi$ ($\xi_n \xrightarrow[]{\text{(r)}} \xi$ или $\xi_n \xrightarrow[]{L_r} \xi$), если
    \begin{equation*}
...
\begin{proof}
    ...
    \item[$\text{p} \implies \text{w}$]
        ...
        \begin{multline*}
            ...
            = \Exp \biggl[ \bigl| f\left(\xi_{n}\right)-f(\xi)\bigr| \cdot \left(\Ind(A_{1})+\Ind(A_{2})+\Ind(A_{3})\right) \biggr]\leqslant \\
    ...
    \item[$\text{p} \notimpliedby \text{d}$]
        ...
        \begin{equation*}
            \MyPr\left({|\xi_n - \xi| > \frac{1}{3}}\right) = \frac{1}{2} \; {\nrightarrow} \; 0 \text{ при } n \to \infty.
        \end{equation*}
...
\begin{rmrk}
    Слабая сходимость всё же не есть сходимость случайных величин, и ею нельзя оперировать как сходимостями п.н. и по вероятности, для которых предельная случайная величина единственна (с точностью до значений на множестве нулевой вероятности).
\end{rmrk}
