\section{Условное математическое ожидание}
    ...
    Вспомним определение условной вероятности: ${\MyPr\left( \xi = x_i | \eta = y_j \right) = \frac{\MyPr\left( \xi = x_i, \, \eta = y_j\right)}{\MyPr\left(\eta = y_j\right)}}$.
    ...
    \begin{equation}
        ...
        \sum\limits_i x_i \, \MyPr\left(\xi = x_i | \eta = y_j \right) \equiv
        f(y_j).
        \label{cond_exp:discrete}
    \end{equation}
    
    $f(y)$~--- \textit{регрессия} $\xi$ \textit{на} $\eta$.
    ...
    \begin{equation*}
        \forall B \in \Borel \quad \Exp \bigl[ \Exp \left( \xi | \eta\right) \Ind(\eta \in B)\bigr] = 
        \Exp \bigl[ \xi \cdot \Ind(\eta \in B) \bigr].
    \end{equation*}
    В самом деле,
    \begin{gather*}
        \Exp \bigl[ \Exp \left( \xi | \eta\right) \Ind(\eta \in B)\bigr] = \\
    ...
    \begin{equation*}
        \MyPr\left( \xi < x \, |\, \eta = y\right) = \frac{\MyPr(\xi < x, \, \eta = y)}{\MyPr(\eta = y)}  = \frac{0}{0} = \; ?
    \end{equation*}
    ...
    так как
    \begin{gather*}
        \Exp \left[ \Exp \left(\xi | \eta\right) \Ind(\eta \in B) \right] = 
        \Exp \left[ f(\eta) \Ind(\eta \in B) \right] = 
    ...
    Перейдём теперь к более строгому, формальному определению условного математического ожидания и поймём, зачем мы установили вышеописанный факт.
...
\subsubsection{Определения, примеры}
    ...
    \begin{defn}
        Случайная величина $\eta$ называется $\Alg$-измеримой, если $\forall B \in \Borel \quad \eta^{-1}(B) \in \Alg$.
    \end{defn}
    ...
    \begin{defn}
        ...
        \begin{enumerate}
            ...
            \item Для любого события $C \in \Alg \quad \Exp \bigl[ \Exp \left( \xi | \Alg \right) \Ind(C) \bigr] \; = \; \Exp \bigl[ \xi \, \Ind(C) \bigr]$.
        \end{enumerate}
    \end{defn}
    ...
    \begin{defn}
        ...
        \end{equation*}
    \end{defn}
    Таким образом, величины \eqref{cond_exp:discrete} и \eqref{cond_exp:abs_continuous} удовлетворяют определению и являются выражениями для УМО в дискретном и абсолютно непрерывном случае соответственно.
        ...
        \iffalse        
            \item 
                ...
                \begin{proof}
                    ...
                    \begin{equation*}
                        \Exp (\xi f(\eta) \cdot g(\eta))=\Exp (\xi h(\eta))=\Exp (\widehat{\xi} h(\eta))=\Exp (\zeta \cdot g(\eta)) \qedhere
                    \end{equation*}
                \end{proof}
        \fi
            \item 
    ...
    \begin{equation*}
        \MyPr(B | \Alg) \overset{\text{def}}{=} \Exp \bigl(\Ind(B) | \Alg \bigr).
    \end{equation*}
...
\iffalse
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            h(y)=\int\limits_{\Real} x \frac{f_{\xi, \eta}(x, y)}{f_{\eta}(y)} dx
        \end{equation*}
        ...
        \begin{equation*}
            f(x | y)=\frac{f_{\xi, \eta}(x, y)}{f_{\eta}(y)}=\frac{f_{\xi, \eta}(x, y)}{\int\limits_{\Real} f_{\xi, \eta}(x, y) dx}
        \end{equation*}
    ...
    \end{enumerate}
        ...
        \begin{equation*}
            \Exp (\xi \cdot g(\eta))=\iint_{\Real^{2}} x g(y) f_{\xi, \eta}(x, y) dxdy
        \end{equation*}
        
        Правая часть равна
        \begin{equation*}
            \Exp (h(\eta) g(\eta))=\int\limits_{\Real} h(y) g(y) f_{\eta}(y) dy=\iint_{\Real \Real} x \frac{f_{\xi, \eta}(x, y)}{f_{\eta}(y)} dx \cdot g(y) f_{\eta}(y) dy
        \end{equation*}
