...
\begin{defn}
    \textit{Медианой} $\Med \xi$ распределения случайной величины $\xi$ называется любое из \textit{чисел} $\mu$ таких, что
    \begin{equation*}
...
\begin{proof}
    \begin{enumerate}
        \item Рассмотрим случайную величину $\eta = \xi - \Med\xi$. 
        Очевидно, что $\Med \eta = 0$. 
        ...
        \begin{equation*}
            \begin{cases}
                |\eta - c| - |\eta| = c, & \eta \leqslant 0; \\
                |\eta - c| - |\eta|  \geqslant -c, & \eta > 0.
        \end{cases}
        \end{equation*}
        
        Тогда
        \begin{gather*}
            \Exp \bigl(|\eta - c| - |\eta| \bigr)
            = \Exp \bigl( (|\eta - c| - |\eta|) \cdot \Ind(\eta \leqslant 0) + (|\eta - c| - |\eta|) \cdot \Ind(\eta > 0) \bigr) \\
            \Exp \bigl( |\eta - c| - |\eta| \bigr) \geqslant c~\MyPr(\eta \leqslant 0) - c~\MyPr(\eta > 0).
        \end{gather*}
        Так как $\Med\eta = 0$, то $\MyPr(\eta \leqslant 0) \geqslant \frac{1}{2} \geqslant \MyPr(\eta > 0) \implies \Exp \bigl( |\eta - c| - |\eta| \bigr) \geqslant 0.$
        ...
        \begin{multline*}
            |\Exp \xi - \Med\xi| =
            |\Exp (\Med\xi - \xi)| \leqslant
            {\text{\{св-во (6) матожидания\}}} \leqslant 
            \Exp |\Med\xi - \xi| \leqslant \\ {\text{\{св-во (1) медианы\}}} \leqslant
            \Exp | \Exp \xi - \xi| =
            \Exp \sqrt{|\Exp \xi - \xi|^2} \leqslant
            {\text{\{нер-во Йенсена\}}} \leqslant \\
            \leqslant \sqrt{\Exp | \Exp \xi - \xi|^2} = \sqrt{\Var\xi} = \sigma. \qedhere
        \end{multline*}
    \end{enumerate}
\end{proof}
\begin{defn}
...
В каком-то смысле эту величину можно считать аналогом дисперсии случайной величины, устойчивым к выбросам.
