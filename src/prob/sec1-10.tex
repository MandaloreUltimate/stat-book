...
\begin{rmrk}
    Для функции совместного распределения выполняются свойства, аналогичные одномерному случаю~--- сохраняется монотонность, непрерывность слева по каждой переменной.
    При этом частные функции распределения восстанавливаются по совместной следующим образом:
    \begin{equation*}
        \lim\limits_{\substack{x_{k} \to +\infty \\ k \neq i}}  F(x_{1}, \ldots, x_{i}, \ldots, x_{n}) = F_{i}(x_i), \quad i = \overline{1,n}.
    \end{equation*}
    ...
    \end{align*}
    В самом деле, по определению $F(x_1, x_2) = \MyPr(\xi_1 < x_1, \xi_2 < x_2) = \MyPr(B_1 \cap B_2),$ 
...
\begin{equation*}
    ...
    = \int\limits_{-\infty}^{x_{1}} f_{\xi_{1}}(x) \,d x \qedhere
\end{equation*}
\end{proof}
...
\begin{defn}
    ...
    \begin{equation*}
        \MyPr\left(\xi_{1} \in B_{1}, \ldots, \xi_{n} \in B_{n}\right)=\MyPr\left(\xi_{1} \in B_{1}\right) \cdot \cdots \cdot \MyPr\left(\xi_{n} \in B_{n}\right)
    \end{equation*}
\end{defn}
\begin{namedthm}[Критерий независимости]
    ...
    \begin{equation*}
        F_{\xi_{1}, \ldots, \xi_{n}}\left(x_{1}, \ldots, x_{n}\right)=F_{\xi_{1}}\left(x_{1}\right) \cdot \cdots \cdot F_{\xi_{n}}\left(x_{n}\right).
    \end{equation*}
    В частности, в случае дискретного совместного распределения:
    \begin{equation*}
        \MyPr\left(\xi_{1}=a_{1}, \ldots, \xi_{n}=a_{n}\right)=\MyPr\left(\xi_{1}=a_{1}\right) \cdot \cdots \cdot \MyPr\left(\xi_{n}=a_{n}\right) \quad \forall a_1, \ldots, a_n \in \Real.
    \end{equation*}
...
Дисперсия их суммы в общем случае равна $\Var(\xi+\eta)=\Var \xi+\Var \eta + 2 \bigl( \Exp (\xi \eta)-\Exp \xi \, \Exp \eta \bigr)$. Величина $\Cov(\xi, \eta) \equiv \Exp (\xi \eta)-\Exp \xi \, \Exp \eta$ называется \textit{ковариацией} случайных величин $\xi$ и $\eta$. Если $\xi$ и $\eta$ независимы, то $\Cov(\xi, \eta) = 0$. Обратное, вообще говоря, неверно.
\end{defn}
...
\begin{exmp}
    ...
    \begin{enumerate}
        \item 
            Докажем некоррелированность данных случайных величин.
            \begin{gather*}
            ...
            \end{gather*}
            Следовательно, $\Cov(\eta_1, \eta_2) = 0$.
        \item 
...
\begin{namedthm}[Свойства ковариации]\leavevmode
    \begin{enumerate}
        ...
        \item 
            $\forall a, b \in \Real ~ \Cov(a \xi + b, \eta)=a \Cov(\xi, \eta)$;
        \item 
            $\Cov(\eta + \zeta, \xi) = 
            \Cov(\eta, \xi) + \Cov(\zeta, \xi)$;
        \item 
            $\Cov^2(\xi, \eta) \leqslant \Var\xi \, \Var\eta$,
        
            $\Cov^2(\xi, \eta) = \Var\xi \,\Var\eta \iff \xi \overset{\text{п.н.}}{=} a\eta + b, \; a, b \in \Real$ (аналог неравенства Коши-Буняковского);
    \end{enumerate}
\end{namedthm}
...
\begin{defn}
    ...
    \begin{equation*}
        \rho(\xi, \eta)=\frac{\Cov(\xi, \eta)}{\sqrt{\Var \xi} \sqrt{\Var \eta}}.
    \end{equation*}
\end{defn}
...
\begin{proof}
    ...
    \item 
        ...
        Так как $\Cov(\xi, \eta)=\Cov\left(\xi_{c}, \eta_{c}\right)$, а дисперсия случайной величины не меняется от смещения случайной величины на константу, коэффициент корреляции не изменится.
        
        Далее, т.к. $\Exp \xi_{c}=\Exp \eta_{c}=0$:
        \begin{gather*}
            \Var \xi_{c}=\Exp \xi_{c}^{2}-\left(\Exp \xi_{c}\right)^{2}=\Exp \xi_{c}^{2},~ \Var \eta_{c}=\Exp \eta_{c}^{2}; \\
            \Cov\left(\xi_{c}, \eta_{c}\right)=\Exp \left(\xi_{c} \, \eta_{c}\right) - \Exp \xi_{c} \, \Exp \eta_{c}=\Exp \left(\xi_{c} \, \eta_{c}\right).
        \end{gather*}
        ...
        \begin{multline*}
            ...
            \implies \bigl|\Cov\left(\xi_{c}, \eta_{c}\right)\bigr| \leqslant \sqrt{\Var \xi_{c} \, \Var \eta_{c}}.
        \end{multline*}
        
        По доказанному выше <<стирание>> индексов не изменит коэффициентов.

    \item 
        Доказательство этого свойства целиком опирается на доказательство предыдущего: если выполнилось равенство $|\Cov(\xi, \eta)|=\sqrt{\Var \xi \, \Var \eta}$, 
        ...
        Знак коэффициента корреляции совпадает со знаком ковариации, так как дисперсии по предположению положительны. Выразив $\xi$ через $\eta$, мы можем воспользоваться свойствами ковариации и получить
        \begin{equation*}
            \rho(\xi, \eta) = \rho(a\eta + b, \eta)=
            \frac{\Cov(a\eta + b, \eta)}
            {\sqrt{\Var(a\eta + b) \, \Var\eta}} =
            \frac{a\Cov(\eta, \eta)}
            {\sqrt{a^2\, \Var\eta \, \Var\eta}} = 
            \frac{a\Var\eta}
            {|a|\Var\eta} =
            \operatorname{sign}(a). \qedhere
        \end{equation*}
\end{enumerate}
\end{proof}
