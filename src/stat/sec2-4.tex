...
\begin{exmp}
    ...
    \end{equation*}
    Для любого фиксированного значения параметра $\theta_0$ функция правдоподобия $L(\SampleX, \theta_0)$ является функцией совместного распределения $(X_1, X_2)$:
    \begin{center}
        \begin{tabular}{|c|c|c|}
            \hline
            & $X_1 = 1$ & $X_1 = 0$ \\
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item[$\impliedby$] Пусть теперь функция правдоподобия имеет вид $L(\boldsymbol{x}, \theta)=g\bigl(T(\boldsymbol{x}), \theta\bigr) h(\boldsymbol{x})$. 
            Тогда, если $\boldsymbol{x}$ таково, что $T(\boldsymbol{x})=t$, то:
            \begin{multline*}
...
\begin{exmp}
    ...
    Напоминаем, что плотность распределения $X_{(n)}$ для $U[0, \theta]$~--- это $f(t) = \cfrac{n t^{n-1}}{\theta^n} \, \Ind(0 \leqslant t \leqslant \theta)$.
    \begin{gather*}
