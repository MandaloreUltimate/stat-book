...
\begin{defn}
    ...
    $\Dist_\theta = (\MyPr_{\theta} \colon \theta \in \Theta \subseteq \Real^{m})$.
\end{defn}
...
\begin{rmrk}
    ...
    Однако вероятностная мера, вообще говоря, не связана ни с какой случайной величиной~--- это просто отображение из некоторой $\sigma$-алгебры событий в вещественные числа, удовлетворяющее аксиомам неотрицательности, нормировки и счётной аддитивности.
    ...
    Следует обратить внимание на то, что поскольку распределение зависит от параметра $\theta$, то от него будут зависеть и математическое ожидание, и дисперсия, и прочие числовые характеристики наблюдаемой случайной величины.
...
\begin{exmp}
    Найдём эмпирические функции распределения крайних порядковых статистик.
    \begin{gather*}
        \begin{aligned}
...
\begin{namedthm}[Свойства эмпирической функции распределения]\leavevmode
    \begin{enumerate}
        ...
        \begin{enumerate}[label={\arabic*)}]
            ...
            \item $\VarTh F_{n}^{*}(y)=\frac{F_{\theta}(y)\bigl(1-F_{\theta}(y)\bigr)}{n} \: \leqslant \: \frac{1}{4n}$\, .
...
\begin{proof}\leavevmode
    \begin{enumerate}
        \item 
            ...
            \begin{equation*}
                F_{n}^{*}(y)=\frac{\sum\limits_{i=1}^{n} \Ind(X_{i}<y)}{n} \xrightarrow[n \to \infty]{\text{p}} \ExpTh \Ind(X_{1}<y)=F_{\theta}(y) \quad \AllTh.
            \end{equation*}
        \item 
            Заметим:
            \begin{gather*}
            ...
            \begin{enumerate}[label={\arabic*)}]
                ...
                \begin{equation*}
                    \ExpTh F_{n}^{*}(y) = \ExpTh \, \frac{\sum\limits_{i=1}^{n} \Ind(X_{i}<y)}{n} =\frac{\sum\limits_{i=1}^{n} \ExpTh \Ind(X_{i}<y)}{n}=\frac{n \ExpTh \Ind(X_{1}<y)}{n}=F_{\theta}(y)  
                \end{equation*}
                
                \item Случайные величины $\Ind(X_{i}<y)$ независимы и одинаково распределены, поэтому:
                \begin{multline*}
                    \VarTh F_{n}^{*}(y)
                    = \VarTh \, \frac{\sum\limits_{i=1}^{n} \Ind(X_{i}<y)}{n}
                    = \frac{\sum\limits_{i=1}^{n} \VarTh \Ind(X_{i}<y)}{n^{2}}
                    = \\
                    = \frac{n\VarTh \Ind(X_{1}<y)}{n^{2}}
                    = \frac{F_{\theta}(y)\bigl(1-F_{\theta}(y)\bigr)}{n}
                \end{multline*}
                Значения $F_{\theta}(y)$ принадлежат отрезку $[0, 1]$, а значит, произведение $F_{\theta}(y)\bigl(1 - F_{\theta}(y)\bigr) \leqslant \frac{1}{2}\cdot\left(1 - \frac{1}{2}\right) = \frac{1}{4}~$ (нетрудно убедиться, что 1/2~--- точка максимума). 
                А значит, $\VarTh F_{n}^{*}(y) \leqslant \frac{1}{4n}\,$. 
            \end{enumerate}
        \item 
            Применим ЦПТ:
            \begin{multline*}
                \sqrt{n}\bigl( F_{n}^{*}(y)-F_{\theta}(y) \bigr)
                = \sqrt{n}\left(\frac{\sum \Ind(X_{i}<y)}{n}-F_{\theta}(y)\right) 
                = \\
                = \frac{\sum\limits_{i=1}^{n} \Ind(X_{i}<y)-n F_{\theta}(y)}{\sqrt{n}} 
                = \frac{\sum\limits_{i=1}^{n} \Ind(X_{i}<y)-n \ExpTh\Ind(X_{1}<y)}{\sqrt{n}} 
        ...
        \item 
            ...
            \begin{equation*}
                n F_{n}^{*}(y)=\Ind\left(X_{1}<y\right)+\cdots+\Ind\left(X_{n}<y\right)
            \end{equation*}
