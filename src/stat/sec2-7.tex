...
В этом случае, при условии обратимости функции~$h$ на~$\Theta$:
\begin{equation*}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        h^{-1}\bigl(h(\theta)\bigr) = \theta. \qedhere
    \end{equation*}
\end{proof}
...
\begin{rmrk}
    ...
    \begin{equation*}
        ...
        H\left(\cfrac{g\left(X_{1}\right)+\cdots+g\left(X_{n}\right)}{n}\right)
    \end{equation*}
...
\begin{proof}
    ...
    \begin{equation*}
        \sigma^{2}(\theta)=\bigl(H^{\prime}(a)\bigr)^{2} \cdot \VarTh g\left(X_{1}\right). \qedhere
    \end{equation*}
\end{proof}
