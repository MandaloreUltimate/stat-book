...
\begin{defn}
    \textit{Несмещённая оценка} параметра $\theta$~--- статистика $T(\SampleX)$, т.ч. $\AllTh\colon \ExpTh T(\SampleX) = \theta$.
\end{defn}
...
\begin{rmrk}
    ...
    \begin{enumerate}
        ...
        К примеру, в качестве несмещённой оценки для математического ожидания $\ExpTh X$ могут выступать $X_{1}$ или $\SampleMean$.
        
        \item Несмещённые оценки могут не существовать.
        \begin{exmp}
            Дано распределение $\Pois_{\theta}$, над которым произведено одно наблюдение. Найти несмещённую оценку для функции $\tau(\theta) = \frac{1}{\theta}$.
                \begin{gather*}
    ...
    \begin{exmp}
        Дано геометрическое распределение $\GeomZero_{1 - \theta}$ с вероятностью успеха $1 - \theta$, над которым произведено одно наблюдение.
...
\begin{proof}
    ...
    Тогда в силу их несмещённости: $\ExpTh  T_{1}(\SampleX) = \ExpTh T_{2}(\SampleX) = \theta$,
    ...
    \begin{equation*}
        T_{3}(\SampleX)=\frac{T_{1}(\SampleX)+T_{2}(\SampleX)}{2}
    \end{equation*}

    Так как $\ExpTh  T_{3}(\SampleX)=\frac{\ExpTh  T_{1}(\SampleX)+\ExpTh  T_{2}(\SampleX)}{2}=\theta$, то $T_{3}(\SampleX)$~--- несмещённая оценка параметра $\theta$.

    Имеем также:
    \begin{equation*}
        \VarTh  T_{3}(\SampleX) = 
        \frac{\VarTh \bigl(T_{1}(\SampleX)+T_{2}(\SampleX)\bigr)}{2^2} =
        \frac{\VarTh  T_{1}(\SampleX)+\VarTh  T_{2}(\SampleX)+2 \Cov\bigl(T_{1}(\SampleX), T_{2}(\SampleX)\bigr)}{4}
    \end{equation*}

    %В силу свойства
    %\begin{equation*}
    %    \ExpTh \xi^{2}<\infty, \ExpTh \eta^{2}<\infty \implies|\Cov(\xi, \eta)| = | \ExpTh (\xi-\ExpTh  \xi)(\eta-\ExpTh  \eta)| \leqslant \sqrt{\VarTh  \xi} \sqrt{\VarTh  \eta},
    %\end{equation*}
    По свойствам ковариации $| \Cov(\xi, \eta)| \leqslant \sqrt{\VarTh \!\xi \, \VarTh \!\eta}$, а значит
    %где равенство достигается тогда и только тогда, когда $\xi=a \eta+b$, получаем:
    \begin{equation*}
        \VarTh  T_{3}(\SampleX) \leqslant \frac{\VarTh  T_{1}(\SampleX)+\VarTh  T_{2}(\SampleX)+2 \sqrt{\VarTh  T_{1}(\SampleX)} \sqrt{\VarTh  T_{2}(\SampleX)}}{4} =\VarTh  T_{1}(\SampleX)
    \end{equation*}
    ...
    \begin{aligned}
        ...
        \iff \\
        \iff \theta = a \theta + b~ \AllTh \implies a = 1, b = 0 \qedhere
    \end{aligned}
    \end{equation*}
\end{proof}
