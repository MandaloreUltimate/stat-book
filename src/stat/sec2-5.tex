...
\vspace{5mm}
Пусть $\Sample$  —  некоторая выборка с функцией правдоподобия $L(\SampleX, \theta)$ относительно некоторой меры $\mu$, доминирующей рассматриваемое семейство распределений. 
Введём функцию ${\varphi(\theta)=\int\limits_{\Real^{n}} T(x) L(\mathbf{x}, \theta) \mu(d x)<\infty}$, в дальнейшем считая, что она дифференцируема необходимое число раз.
