...
Рассмотрим достаточно общий способ построения кратчайших доверительных интервалов.
    ...
    \item %Из леммы следует, что для любого $\theta$ в абсолютно непрерывном случае
        ...
        \begin{equation*}
            \begin{aligned}
                ...
                = \cfrac{1-\gamma}{2} \\
                \implies
                \MyPrTh \Bigl(\underbrace{\varphi_{1} \bigl(T(\SampleX)\bigr)}_{T_{1}(\SampleX)}
                \leqslant \theta
                \leqslant \underbrace{\varphi_{2}\bigl(T(\SampleX)\bigr)}_{T_{2}(\SampleX)} \Bigr)
                = \gamma
            \end{aligned}
        \end{equation*}
